...
class ModelBuilder(BaseModelBuilder):
    ...
    # TODO добавить чтение параметра параметра поворота вокруг ox
    def read_muscle(self, file):
        point_start = None
        point_end = None
        a = None
        b = None
        ...
        rotation_vector = Vector(0, angle_oy, angle_oz)

        result = [a, b, l, center, rotation_vector]

        return result

    def read_tendon(self, file):
        point_start = None
        point_end = None
        s = None
        while 1:
        ...
        if (s is None) or (point_start is None) or (point_end is None):
            # TODO error
            quit()
